...
def login(
    ...
    member = db.query(models.Member).filter(
        models.Member.email == form_data.username
    ).first()
    if not member or not verify_password(
        form_data.password, member.hashed_password
    ):
        raise HTTPException(
...
def remove_book(
    ...
    if active_loan:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"Cannot remove '{db_book.title}' - "
                f"it is currently on loan!"
            )
        )
...
def loan_book(
    ...
    target_member_id = (
        loan_in.member_id
        if loan_in.member_id is not None
    ...
    if (
        ...
        and target_member_id != current_user.id
    ):
        raise HTTPException(
    ...
    member = db.query(models.Member).filter(
        models.Member.id == target_member_id
    ).first()
    if not member:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def notify_overdue_loans(
    ...
    """Trigger background overdue book notifications (Librarian only)."""
    ...
    for loan in active_loans:
        book = db.query(models.Book).filter(
            models.Book.id == loan.book_id
        ).first()
        member = db.query(models.Member).filter(
            models.Member.id == loan.member_id
        ).first()
